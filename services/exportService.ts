...
// === EXCEL IMPORT ===
export const parseExcelImport = async (
  file: File, 
  categories: Category[], 
  accounts: Account[]
): Promise<{ transactions: Transaction[], updatedCategories: Category[] }> => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const parsedTransactions: Transaction[] = jsonData.map((row: any) => {
            ...
            let foundCategory = updatedCategories.find(c => 
              c.name.toLowerCase() === categoryName.toLowerCase() && c.type === type
            );

            if (!foundCategory && categoryName && type !== 'transfer') {
                foundCategory = {
                ...
                updatedCategories.push(foundCategory);
            }

            if (foundCategory && subCategoryName) {
                const hasSub = foundCategory.subCategories.some(s => s.toLowerCase() === subCategoryName.toLowerCase());
                if (!hasSub) {
                    foundCategory.subCategories.push(subCategoryName);
                }
            }
            // ================================
        ...
        resolve({ 
            transactions: parsedTransactions.filter(t => t.amount > 0),
            updatedCategories
        });
      } catch (err) {
        reject(err);
